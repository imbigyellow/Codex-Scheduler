...
- Codex 后台续聊任务保存会话 UUID 和工作目录。Helper 使用已登录的 Codex CLI 的 `exec resume`，经 stdin 传入提示词，在 `workspace-write` 沙箱和 `never` 审批策略下执行。此路径不依赖前台窗口或辅助功能权限。等待此类任务期间可选用 `caffeinate -i -w <helper PID>` 阻止闲置睡眠；Helper 退出时该断言自动结束。它不阻止用户主动睡眠或合盖。
...
电脑重启后，用户重新登录时 LaunchAgent 会被系统加载。UI 发送任务若迟到超过 10 分钟，后台续聊任务若迟到超过 30 秒，均标记为“已错过”。Helper 监听系统将睡眠和已唤醒通知；睡眠跨过目标时间时直接标记“已错过”，不会补发。睡眠开始时间写入磁盘，后台服务重启后仍可完成标记。目标时间是创建时的绝对时间，更改系统时区不会改变它。本应用没有系统级唤醒权限，也不能保证真正睡眠期间准点执行。后台模式依赖本机 Codex CLI 已登录、工作目录存在、会话 ID 正确且没有并发运行的任务。辅助功能发送仍受锁屏、目标 App UI 状态与系统 TCC 策略影响。
